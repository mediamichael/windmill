<script lang="ts">
    ...
    const { app, mode, worldStore, selectedComponent } = getContext<AppViewerContext>('AppViewerContext');
    const outputs = initOutput($worldStore, id, { loading: false });
    ...
    $: {
        if (controlsWidth && controlsWidth > 450) {
            wideView = true;
        } else {
            wideView = false;
        }
    ...
    async function renderPdf(scaleToViewport = true, resizing = false) {
        ...
        wrapper.scrollTo({ top: scrollPosition * wrapper.scrollHeight });
    }

    function scrollToPage(page: number) {
        page = pageNumber = Math.min(Math.max(page, 1), pages.length);
        ...
        wrapper?.scrollTo({ top: offset - padding });
    }
    ...
    function onScroll() {
        ...
        for (let i = 0; i < pages.length; i++) {
            ...
            if (scrollPosition < (canvas?.offsetTop ?? wrapper.scrollHeight)) break;
            page = i + 1;
        }
        pageNumber = page;
    }
    ...
    function syncZoomValue() {
        ...
        if (gridItem && gridItem.data.configuration.zoom.value !== zoom) {
            ...
            $app = $app; // Trigger Svelte reactivity
        }
    }
