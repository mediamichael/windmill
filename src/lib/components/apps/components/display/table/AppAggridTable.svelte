<script lang="ts">
	import { createGrid, Grid } from 'ag-grid-community';
	...
	function onSelectionChanged(event: { api: Grid }) {
		const api = event.api;
		if (!api) return;  // Safety check to ensure API is defined

		if (resolvedConfig?.multipleSelectable) {
			const rows = api.getSelectedNodes();  // Get all selected nodes
			if (rows.length > 0) {
				toggleRows(rows);
			}
		} else {
			const row = api.getSelectedNodes()[0];  // Get the first selected node
			if (row) {
				toggleRow(row);
			}
		}
