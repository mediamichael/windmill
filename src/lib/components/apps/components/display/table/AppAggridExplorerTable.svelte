<script lang="ts">
	import { Grid, createGrid, type IDatasource, type GridOptions } from 'ag-grid-community';
	...
	let api: Grid<any> | undefined = undefined;
	...
	function mountGrid() {
		if (eGui) {
			const gridOptions: GridOptions = {
				...
				maxBlocksInCache: 20,
				rowHeight: resolvedConfig?.wrapActions ? Math.max(44, actions.length * 48) : 44,
	...
	$: if (!deepEqual(extraConfig, resolvedConfig.extraConfig)) {
		...
		if (extraConfig) {
			api?.setGridOptions(extraConfig);
		}
	}
	...
	function onSelectionChanged(event: { api: Grid }) {
		const api = event.api;
