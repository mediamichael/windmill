  ...
  if (t === "int") {
    ...
    if (t.oneof) {
      newS.oneOf = t.oneof.map((obj) => {
        const oldObjS = oldS.oneOf?.find((o) => o?.title === obj.label) ?? void 0;
        const properties = {};
    ...
    if (t.list === "int" || t.list === "float") {
      ...
      newS.items = { type: "resource", resourceType: t.list.resource };
    } else if (t.list && typeof t.list == "object" && "object" in t.list && t.list.object && t.list.object.length > 0) {
      const properties = {};
      for (const prop of t.list.object) {
        properties[prop.key] = { description: "", type: "" };
        argSigToJsonSchemaType(prop.typ, properties[prop.key]);
      }
      newS.items = { type: "object", properties };
    } else {
